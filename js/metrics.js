...
(function (root) {
  ...
  //
  // ИСПРАВЛЕНО (Дима, 2026-09-08 -- см. чат/HISTORY.md): раньше здесь был спецкейс
  // "if (atDate < kassa.appearance) return false" -- дата раньше появления кассы трактовалась
  // как "не в оттоке". Для ОДНОЙ кассы, проверяемой изолированно на дату ПОСЛЕ её собственного
  // появления (все реальные вызывающие места вызывают её именно так -- ctx.asOf "сейчас",
  // graceDeadline после overallEnd, или явный guard appearance<periodStart у вызывающего) этот
  // спецкейс был no-op. Но там же, где он был единственной защитой (историческая дата ДО
  // появления кассы -- activeCountsAtMonthEnds/computeSnapshot для прошлых месяцев в "Прирост
  // базы"), он ломал результат: возвращая false ("не лапсед"), функция фактически говорила
  // "касса активна", хотя её физически ещё не было -- на реальных данных (2022-01, файл с
  // историей 2022-2026) это давало 214338 "активных касс" вместо истинных 3325 (98.4% --
  // ложное срабатывание через ЭТУ строку). Через clientLapsedAt (OR по всем кассам клиента)
  // эффект каскадировался ещё сильнее: 127186 "активных клиентов" вместо 2617. Просто убрать
  // строку -- проверено эмпирически, что на asOf ("сейчас") результат не меняется НИ НА ОДНУ
  // кассу/клиента (все реальные кассы уже появились к текущему моменту по определению) --
  // задеты только исторические (прошлые) даты. clientLapsedAt не трогаем -- она использует эту
  // функцию внутри, чинится автоматически.
