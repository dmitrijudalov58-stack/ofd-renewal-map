...
function extractFields(resp) {
  ...
  return {
    ...
    // ОПФ (юр.форма, "ООО"/"ИП"/...) -- 5-й признак скоринга (Дима, 2026-09-18): TVD=0.30 на
    // реальных данных 2025 года, сильнее уже используемого "числа касс" (0.23) -- см.
    // tmp/plans/2026-09-18-scoring-weights-plan.md. Записи, обогащённые ДО этого поля,
    // бэкфилятся отдельным разовым скриптом из уже скачанного dadata-raw.ndjson (полный
    // ответ там уже есть) -- без повторных запросов к DaData.
    opf: (d.opf && d.opf.short) || null,
  };
}
